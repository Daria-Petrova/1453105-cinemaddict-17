...
import { getHours, getMinutes, getHumaneDate , getRelativeDate} from '../utils.js';
...
const createDetailedInfoTemplateView = (film = {}, commentList = {}) => {
  const {filmInfo, comment } = film.films[0];
  const {comments} = commentList;

  const infoTemplate = createFilmDetailsInfo(filmInfo);
  const commentListTemplate = createCommentList(comments, comment);
  const commentCount = countingComments(comments, comment);
